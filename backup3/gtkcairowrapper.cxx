...
void FontData::Print(const String &Text, TextAlignment Alignment, const FlatVector &Position)
{
...
void FontData::Print(const String &Text, TextAlignment Alignment, const FlatVector &Position, const FlatVector &Limits)
{
	PangoLayout *Layout = pango_cairo_create_layout(Canvas->GetContext());
	pango_layout_set_font_description(Layout, PangoFont);
	pango_layout_set_text(Layout, Text.c_str(), -1);
...
{
	gtk_widget_add_events(Data, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK | GDK_POINTER_MOTION_MASK | GDK_POINTER_MOTION_HINT_MASK );
...
void VectorArea::SetColor(const Color &NewColor)
	{ cairo_set_source_rgb(CairoContext, NewColor.Red, NewColor.Green, NewColor.Blue); }
...
void VectorArea::DrawText(const String &Text, TextAlignment Alignment, const FlatVector &Position)
{
	cairo_text_extents_t Extents;
	cairo_text_extents(CairoContext, Text.c_str(), &Extents);
	...
	if (Alignment == taMiddle)
		...
		Start[0] -= Extents.x_advance;*/
	FlatVector Start = Position + FlatVector(-Extents.x_bearing, -Extents.y_bearing);
	if (Alignment == taMiddle)
		Start[0] -= Extents.width * 0.5f;
	else if (Alignment == taRight)
		Start[0] -= Extents.width;

	cairo_set_source_rgb(CairoContext, 0, 0, 0);
	//cairo_move_to(CairoContext, Start[0], Start[1] + 18.0f);
	cairo_move_to(CairoContext, Start[0], Start[1]);
	cairo_show_text(CairoContext, Text.c_str());
}

void VectorArea::DrawText(const String &Text, const FlatVector &Alignment, const FlatVector &Position)
{
	cairo_text_extents_t Extents;
	cairo_text_extents(CairoContext, Text.c_str(), &Extents);
...
void VectorArea::ResizeEvent(const FlatVector &NewSize) {}
void VectorArea::Draw(void) {}
void VectorArea::ClickEvent(const FlatVector &Cursor, bool LeftChanged, bool MiddleChanged, bool RightChanged) {}
void VectorArea::DeclickEvent(const FlatVector &Cursor, bool LeftChanged, bool MiddleChanged, bool RightChanged) {}
void VectorArea::MoveEvent(const FlatVector &Cursor) {}
...
void VectorArea::DrawInternal(int X, int Y, int Width, int Height)
{
	...
	cairo_set_font_size(CairoContext, 12);
	//std::cout << "Cairo context from " << X << ", " << Y << " size " << Width << " x " << Height << std::endl;
...
gboolean VectorArea::ClickHandler(GtkWidget *Widget, GdkEventButton *Event, VectorArea *This)
{
	...
	This->ClickEvent(FlatVector(Event->x, Event->y),
