...
void MenuItem::Disable(void)
	{ gtk_widget_set_sensitive(Data, false); }

void MenuItem::Enable(void)
...
void ToolButton::Disable(void)
	{ gtk_widget_set_sensitive(Data, false); }

void ToolButton::Enable(void)
	{ gtk_widget_set_sensitive(Data, true); }
...
void RatioFrame::Set(GtkWidget *Settee)
	{ gtk_container_add(GTK_CONTAINER(Data), Settee); gtk_widget_show(Settee); }
...
int Notebook::Add(GtkWidget *Addee, const String &Title)
{
	Label *TabLabel = new Label(Title);
	int Page = gtk_notebook_insert_page(GTK_NOTEBOOK(Data), Addee, *TabLabel, -1);
...
// Button wrapper
Button::Button(ActionHandler *Handler, const String &Text) :
...
Button::Button(ActionHandler *Handler, const String &Text, DefaultIcons Icon) :
	Widget(Text.empty() ? gtk_button_new() : gtk_button_new_with_label(Text.c_str())),
	Handler(Handler),
	ConnectionID(g_signal_connect(G_OBJECT(Data), "clicked", G_CALLBACK(PressHandler), this)),
	LastIcon(diNone)
...
ShortEntry::ShortEntry(Handler *Target, const String &InitialText) : Widget(gtk_entry_new()),
	Target(Target), ConnectionID(g_signal_connect(G_OBJECT(Data), "changed", G_CALLBACK(EntryHandler), this))
{
	if (!InitialText.empty())
		gtk_entry_set_text(GTK_ENTRY(Data), InitialText.c_str());
}
...
Slider::Slider(Handler *Target, float Minimum, float Maximum, float InitialValue) :
	...
	Target(Target), ConnectionID(g_signal_connect(G_OBJECT(Data), "value-changed", G_CALLBACK(SlideHandler), this))
...
Wheel::Wheel(Handler *Target, int Min, int Max, int Initial) :
	Widget(gtk_spin_button_new_with_range(Min, Max, Initial)),
	Target(Target), ConnectionID(g_signal_connect(G_OBJECT(Data), "value-changed", G_CALLBACK(SpinHandler), this))
	{}
	...
	Store(gtk_list_store_new(1, G_TYPE_STRING))
{
	...
	if (Multiline)
	{
		...
		gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(Data), false);
	...
	else
	{
		...
		ConnectionID = g_signal_connect(G_OBJECT(Data), "changed", G_CALLBACK(SelectHandler), this);
	}
}
...
void MenuButton::Act(void *Source)
...
// File things
OpenSelect::OpenSelect(ActionHandler *Handler, const String &InitialDirectory) :
	Widget(gtk_file_chooser_button_new("Select directory", GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER)),
	Handler(Handler), ConnectionID(g_signal_connect(G_OBJECT(Data), "file-set", G_CALLBACK(OpenHandler), this))
{
	gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(Data), InitialDirectory.c_str());
	SingleFilter = NULL;
}

OpenSelect::OpenSelect(ActionHandler *Handler, const String &InitialFile, const String &FilterName) :
...
OutputSelect::OutputSelect(Handler *Target, const String &InitialDirectory) : Layout(true),
...
void OutputSelect::Act(void *Source)
{
...
ToolButton *Toolbar::Add(ToolButton *NewItem)
{
	gtk_toolbar_insert(GTK_TOOLBAR(Data), GTK_TOOL_ITEM((GtkWidget *)*NewItem), -1);
	gtk_widget_show(*NewItem);
	return NewItem;
}

void Toolbar::Add(String const &Text, GtkWidget *Item)
{
	GtkWidget *ItemBox = gtk_hbox_new(false, 3);

	GtkWidget *TextLabel = gtk_label_new(Text.c_str());
	gtk_box_pack_start(GTK_BOX(ItemBox), TextLabel, false, true, 0);
	gtk_widget_show(TextLabel);
	gtk_box_pack_start(GTK_BOX(ItemBox), Item, true, true, 0);
	gtk_widget_show(Item);

	GtkToolItem *NewToolItem = gtk_tool_item_new();
	gtk_container_add(GTK_CONTAINER(NewToolItem), ItemBox);
	gtk_widget_show(ItemBox);

	gtk_toolbar_insert(GTK_TOOLBAR(Data), NewToolItem, -1);
